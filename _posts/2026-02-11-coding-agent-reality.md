...
> **요약**
> - AI 코딩 에이전트를 도입하면 코드 작성 속도는 빨라지지만, 리뷰·검증·아키텍처 판단 부담도 함께 커진다.
> - 에이전트 PR 생성 → CI 자동 리뷰 → 사람 최종 판단으로 이어지는 실제 개발 워크플로우를 공유한다.
> - 핵심은 자동화 자체가 아니라, 에이전트가 만든 결과를 검증할 수 있는 일의 흐름을 설계하는 것이다.
...
판단의 핵심 역량에 대해서는 [AI 시대에 문제 정의 능력이 더 중요해진 이유](/posts/importance-of-problem-definition/)에서 따로 다뤘다.
...
### AI 코딩 에이전트를 도입하면 실제로 생산성이 10배 오르나요?
구현 속도는 크게 올라갈 수 있지만 전체 개발 생산성이 그대로 10배 오르는 것은 아닙니다. AI가 만든 코드는 리뷰, 테스트, 아키텍처 검토가 필요하고 이 검증 속도는 코드 생성 속도만큼 빨라지지 않습니다. 따라서 생산성 향상은 자동화된 구현과 사람이 맡는 판단 과정을 함께 설계할 때 현실화됩니다.

### AI 에이전트를 활용한 실무 개발 워크플로우는 어떻게 구성하나요?
에이전트가 PR을 생성하면 CI가 자동으로 리뷰합니다. 피드백을 반영해 재리뷰 사이클을 반복한 뒤, 마지막에 사람이 전수검사를 합니다. 아키텍처가 근본적으로 잘못된 경우 PR을 닫고 새로 시작하는 편이 낫습니다. 에이전트를 채용 후보처럼 코드를 리뷰하는 감각이 중요합니다.

### AI가 만든 코드를 효과적으로 검토하는 방법은 무엇인가요?
핵심 기능의 동작 여부를 먼저 확인하고, 아키텍처 전체를 조감한 뒤 세부 코드를 읽는 순서가 효과적입니다. 파일 하나하나는 그럴듯해도 전체 구조가 뒤엉겨 있는 경우가 많습니다. 반복되는 실수 패턴을 파악해 가이드라인 문서와 프롬프트 단에서 미리 차단하는 하네스 엔지니어링 접근도 도움이 됩니다.
